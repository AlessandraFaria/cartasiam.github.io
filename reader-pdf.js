...
export function initPdfReader({ book, manifestUrl, baseFolder }) {
  ...
  function computeSinglePageSize() {
    ...
    const isStacked = vw <= 720; // bate com o breakpoint do CSS — sidebar vira faixa no topo
    const sidebarSize = sidebar ? (isStacked ? sidebar.offsetHeight : sidebar.offsetWidth) : 0;

    // no layout empilhado (celular) a sidebar consome ALTURA; no layout lateral
    // (desktop) ela consome LARGURA — e reservamos um respiro pras setas de navegação.
    const availH = isStacked
      ? Math.max(240, vh - sidebarSize - 24)
      : Math.max(320, vh - 32);
    const availW = isStacked
      ? Math.max(200, vw - 24 - 70)
      : Math.max(220, vw - sidebarSize - 130);
